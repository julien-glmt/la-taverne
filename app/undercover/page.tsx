...
export default function UndercoverLobby() {
  ...
  const [step, setStep] = useState<"avatar" | "choose" | "create" | "join">(
    "avatar",
  );
  ...
  function handleCreate() {
    ...
    router.push(
      `/undercover/${code}?name=${encodeURIComponent(name.trim())}&avatar=${encodeURIComponent(avatar)}&host=true`,
    );
  }
  ...
  function handleJoin() {
    ...
    router.push(
      `/undercover/${joinCode.toUpperCase()}?name=${encodeURIComponent(name.trim())}&avatar=${encodeURIComponent(avatar)}`,
    );
  }
  ...
  return (
    <main className="min-h-screen bg-[#1a1208] text-[#e8dcc8] font-sans flex flex-col">
      ...
      <div className="relative z-10 max-w-md mx-auto w-full px-6 py-12 flex flex-col flex-1">
        <a
          href="/"
          className="text-xs text-[#4a3820] hover:text-[#c8a030] tracking-widest uppercase mb-12 inline-block transition-colors"
        >
          ← La Taverne
        </a>
        ...
        <div className="text-center mb-10">
          ...
          <h1
            className="text-3xl text-[#f0e0b0] mb-2"
            style={{ fontFamily: "Georgia, serif", fontWeight: 400 }}
          >
            Undercover
          </h1>
          <p className="text-sm text-[#6a5838]">
            Trouve l'imposteur avant qu'il te démasque.
          </p>
        </div>
          ...
          <div>
            <div className="mb-6">
              ...
              <input
                ...
                onFocus={(e) =>
                  (e.target.style.borderColor = "rgba(200,160,48,0.4)")
                }
                onBlur={(e) =>
            ...
            <div className="mb-8">
              <label className="block text-xs tracking-[0.2em] uppercase text-[#4a3820] mb-3">
                Choisis ton avatar
              </label>
            ...
            {error && (
              <p className="text-xs text-[#c87050] mb-4 text-center">{error}</p>
            )}
        ...
        {step === "choose" && (
          <div>
            <div
              className="flex items-center gap-3 mb-8 p-4 rounded-sm"
              style={{
            ...
            >
              ...
              <div>
                ...
                <button
                  ...
                  className="text-xs text-[#4a3820] hover:text-[#c8a030] transition-colors"
                >
                  Modifier
                </button>
              </div>
        ...
        {step === "join" && (
          <div className="flex flex-col gap-3">
            ...
            {error && (
              <p className="text-xs text-[#c87050] text-center">{error}</p>
            )}
            <button
              onClick={handleJoin}
              className="w-full py-4 rounded-sm text-sm font-medium tracking-wide"
